...
void Bot::AutoChaosLockThread(const unsigned int key, const int count)
{
	while (Variable.Flag.EnableBot)
	{
		if (!Variable.Flag.StopBot)
		{
			if (!Variable.Flag.CastingSkill)
			{
				if (MS.Data.GetAttackCount() >= count)
				{
...
void Bot::AutoFuryAttackThread(const unsigned int key, const int fury)
{
	while (Variable.Flag.EnableBot)
	{
		if (!Variable.Flag.StopBot)
...
void Bot::MuLungDojoThread()
{
	...
	while (Variable.Flag.EnableBot)
	{
		...
		case hall:
			...
			SendPacket("71 00 AF 30 03 00 00 00 00 00"); // Talk to panda in hall
			Sleep(1000);
			...
			MS.PushKey(VK_RETURN);
			Sleep(500);
			SendPacket("DE 00 03 70 1C"); // Reset blocked quest
		...
		case floor:
			...
			prevMap = MS.Data.GetMapId();
			
			for (int i = 0; i < 10; i++)
			{
				if (prevMap == (floorMaps[40] + i))
				{
...
void Bot::CreateAutoChaosLockThread(const unsigned int key, const int count)
{
	new boost::thread(&Bot::AutoChaosLockThread, this, key, count);
}

void Bot::CreateAutoFuryAttackThread(const unsigned int key, const int fury)
{
	new boost::thread(&Bot::AutoFuryAttackThread, this, key, fury);
}
